import { ArrowLeft, Check, FileText, ScrollText } from 'lucide-react'
...
interface UnsecuredPromissoryNoteValues {
...
interface UnsecuredPromissoryNoteFlowProps {
  onBackToInvest: () => void
}
...
export const UnsecuredPromissoryNoteFlow = ({ onBackToInvest }: UnsecuredPromissoryNoteFlowProps) => {
  const [step, setStep] = useState<FlowStep>(2)
  const [values, setValues] = useState<UnsecuredPromissoryNoteValues>({
    borrowerName: '',
    principalAmount: '',
    interestRate: '',
    originationDate: '',
    ...
    if (step === 3) {
      return Boolean(
        values.fundingMethod &&
          ...
          values.purposeOfLoan.trim() &&
          values.confirmsNoCollateral &&
  ...
  const setField = <K extends keyof UnsecuredPromissoryNoteValues>(key: K, value: UnsecuredPromissoryNoteValues[K]) => {
      ...
      <div className="flex min-h-[calc(100vh-3rem)]">
        <aside className="hidden w-[360px] border-r border-border px-5 py-6 lg:block">
          <ol className="space-y-5">
            ...
            <StepItem index={2} label="Note Details" status={step === 2 ? 'active' : 'completed'} />
          ...
          <div className="flex items-center justify-between px-4 py-4 md:px-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-semibold text-text-primary md:text-2xl">
                {step === 2 ? 'Note Details' : step === 3 ? 'Borrower & Compliance' : 'Document Sign'}
              </h2>
            </div>
            ...
            {step === 3 ? (
              <div className="space-y-4 rounded-2xl border border-border p-4 md:border-0 md:p-0">
                <div className="rounded-2xl border border-border p-4">
                  ...
                  <div className="mt-3 space-y-3">
                    {[
                      ['cash', 'Cash available in account'],
                ...
                <label className="block">
                  <span className="mb-2 block text-xs text-text-secondary">Purpose of Loan*</span>
                  <input
                    className={inputClass}
                    placeholder="Describe loan purpose"
                    value={values.purposeOfLoan}
                    onChange={(event) => setField('purposeOfLoan', event.target.value)}
                  />
                </label>

                {[
                  ['confirmsNoCollateral', 'I confirm this note is unsecured and has no collateral.'],
                  ...
                  ['confirmsReviewedTerms', 'I reviewed all note terms and repayment obligations.'],
                ].map(([key, label]) => (
                  <label key={key} className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-3 text-sm text-text-primary">
                    <span
                      className={cn(
                        ...
                        values[key as keyof UnsecuredPromissoryNoteValues]
                          ? 'border-error bg-error text-text-inverse'
                          : 'border-border bg-surface',
                      )}
                    >
                      {values[key as keyof UnsecuredPromissoryNoteValues] ? <Check className="h-3.5 w-3.5" /> : null}
                    </span>
                    {label}
                    <input
                      type="checkbox"
                      className="sr-only"
                      checked={Boolean(values[key as keyof UnsecuredPromissoryNoteValues])}
                      onChange={(event) => setField(key as keyof UnsecuredPromissoryNoteValues, event.target.checked)}
                    />
                  </label>
                ))}
              </div>
              ...
              <div className="space-y-4 rounded-2xl border border-border p-4 md:border-0 md:p-0">
                <div className="rounded-2xl border border-border p-5">
                  <div className="flex min-h-[220px] flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-2xl font-semibold text-text-primary md:text-3xl">Sign Promissory Note Request</h3>
                    <p className="mt-2 text-base text-text-secondary">
                      Digitally sign to submit your un-secured promissory note request.
                    </p>
